...
switch ($opt) {
    ...
    case 'edit':
        $msg = isset($msg) ? $msg : '';
        $id = isset($_REQUEST['ID_US']) ? $_REQUEST['ID_US'] : '';
        $sqlE = 'SELECT C.USERNAME, B.NOMBRE_US, B.APELLIDO_US, B.EMAIL_US, B.TELEFONO_US, B.DIRECCION_US, B.FECHANAC_US
                FROM cat_rol A,cat_usuarios B,login C
                WHERE C.ID_ROL = A.ID_ROL
                AND B.ID_LOG = C.ID_LOG
                AND A.ID_ROL = 2
...
//        $FECHAREGISTRO_US = isset($_REQUEST['FECHAREGISTRO_US']) ? addslashes($_REQUEST['FECHAREGISTRO_US']) : '';
        $sqlV = 'SELECT a.ID_US
                 FROM cat_usuarios A, login B
                 WHERE a.ID_LOG = b.ID_LOG
                 AND b.USERNAME = "'.$USERNAME.'"';
        $qryV = mysql_query($sqlV) or die('Error: ' . mysql_error());
        $qryVResult = mysql_fetch_assoc($qryV) or die('Error: ' . mysql_error());
        $idtemp = $qryVResult['ID_US'];
        $sqlU = "UPDATE cat_usuarios SET NOMBRE_US= '$NOMBRE_US', APELLIDO_US= '$APELLIDO_US', FECHANAC_US= '$FECHANAC_US', TELEFONO_US= '$TELEFONO_US', DIRECCION_US= '$DIRECCION_US',EMAIL_US= '$EMAIL_US' WHERE ID_US= '$idtemp'";
    ...
    default:
        ...
        $sqlL = 'SELECT C.USERNAME, B.NOMBRE_US, B.APELLIDO_US, B.EMAIL_US, B.TELEFONO_US, B.DIRECCION_US, B.FECHANAC_US
                FROM cat_rol A,cat_usuarios B,login C
                WHERE C.ID_ROL = A.ID_ROL
                AND B.ID_LOG = C.ID_LOG
                AND A.ID_ROL = 2
